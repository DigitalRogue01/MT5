//+------------------------------------------------------------------+
//|                  Camarilla_H4_Sweep_Reclaim_Short_EA_v1_00.mq5   |
//| Focused Camarilla H4 sweep reclaim short EA                      |
//+------------------------------------------------------------------+
#property strict
...
input double NearH4ATRDistance        = 0.25;
...
bool EnsureEMA20Handle()
{
   if(g_ema20Handle != INVALID_HANDLE)
      return true;
   g_ema20Handle = iMA(_Symbol, PERIOD_CURRENT, EMAPeriod, 0, MODE_EMA, PRICE_CLOSE);
...
double GetCamarillaH4(const int shift=1)
{
   if(!EnsureCamarillaHandle())
      return 0.0;
   return ReadIndicatorValue(g_camarillaHandle, 8, shift);
}
...
void InitLogFile()
{
   ...
   g_logFileName = StringFormat("Camarilla_H4_Sweep_Reclaim_Short_EA_v1_00_%s.csv", CurrentMonthName());
   ...
   {
      string header =
         "Time,Symbol,TF,Bid,Ask,H4,VWAP,EMA20,SpreadPoints,SellScore,Decision,Reason,ATR,Lots,SL,TP\r\n";
...
void LogNoTradeReason(const StrategyContext &ctx,const int sellScore,const string reason)
{
   g_lastStatus = reason;
   LogLine(ctx, sellScore, "WAIT", reason, 0.0, 0.0, 0.0);
}

void LogNearSetup(const StrategyContext &ctx,const int sellScore,const string reason)
{
   g_lastStatus = reason;
   LogLine(ctx, sellScore, "NEAR", reason, 0.0, 0.0, 0.0);
}
...
bool DescribeNearSetup(const StrategyContext &ctx,
                       const string lastReason,
                       string &nearReason)
{
   nearReason = "";

   if(ctx.atr <= 0.0 || ctx.h4 <= 0.0)
      return false;

   if(!DRCSM_IsAllowedSession(ctx.hour, UseLondonSession, UseNewYorkSession))
      return false;

   double closeDistanceAtr = MathAbs(ctx.bar1.close - ctx.h4) / ctx.atr;
   double highOvershootAtr = (ctx.bar1.high - ctx.h4) / ctx.atr;
   bool belowEma = (ctx.ema20 > 0.0 && ctx.bar1.close < ctx.ema20);

   if(lastReason == "SKIP_NO_RECLAIM" && ctx.bar1.high >= ctx.h4)
   {
      nearReason = "NEAR_SWEEPED_H4_NO_RECLAIM";
      return true;
   }

   if(lastReason == "SKIP_SCORE_TOO_LOW" && ctx.bar1.high >= ctx.h4 && ctx.bar1.close < ctx.h4)
   {
      nearReason = "NEAR_H4_RECLAIM_WEAK_SCORE";
      return true;
   }

   if(belowEma && closeDistanceAtr <= NearH4ATRDistance)
   {
      nearReason = "NEAR_BELOW_EMA20_CLOSE_TO_H4";
      return true;
   }

   if(belowEma && highOvershootAtr >= -NearH4ATRDistance)
   {
      nearReason = "NEAR_BELOW_EMA20_APPROACHING_H4";
      return true;
   }
...
void ManageOpenTrades()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(riskDist <= 0.0 || type != POSITION_TYPE_SELL)
         continue;
...
bool ExecuteShortTrade(const StrategyContext &ctx,
                       const string tag,
                       const int sellScore,
                       string &reason)
{
   ...
   double stopAnchor = MathMax(ctx.bar1.high, ctx.h4);
   double sl = stopAnchor + (ctx.atr * ATR_SL_Mult);
   double tp = bid - (sl - bid) * RR_For_TP;
   double lots = CalcLotsFromRisk(bid, sl);

   if(lots <= 0.0)
   {
      reason = tag + "_LOT_INVALID";
      LogLine(ctx, sellScore, "BLOCKED", reason, lots, sl, tp);
      return false;
   }

   if(trade.Sell(lots, _Symbol, bid, sl, tp, tag))
   {
      g_lastTradeBar = ctx.currentBarTime;
      g_lastStatus = tag;
      g_lastSetupSummary = "H4 reclaim short fired";
      LogLine(ctx, sellScore, "SELL", tag, lots, sl, tp);
      return true;
   }
...
void OnTick()
{
   ...
   if(DRCSM_EvaluateH4SweepReclaimShort(ctx.moduleCtx,
                                        UseLondonSession,
                                        UseNewYorkSession,
                                        MinBodyToRange,
                                        MinSweepATRFrac,
                                        ...
                                        signal))
   {
      sellScore = signal.score;
      g_lastSetupSummary = signal.summary + " | Score " + IntegerToString(signal.score);
      if(ExecuteShortTrade(ctx, signal.tag, signal.score, reason))
      {
         UpdateComment();
         return;
      }
   ...
   if(!signal.detected)
      g_lastSetupSummary = "No setup";
   if(DescribeNearSetup(ctx, reason, nearReason))
   {
      g_lastSetupSummary = "Near setup";
      LogNearSetup(ctx, sellScore, nearReason);
   }
   else if(LogEachCandle)
   {
      LogNoTradeReason(ctx, sellScore, reason);
   }
   UpdateComment();
}
